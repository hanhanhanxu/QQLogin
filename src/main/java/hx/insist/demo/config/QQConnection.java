...
public class QQConnection {
    ...
    public String getAccessToken(String code){
        ...
        try (Response response = client.newCall(request).execute()) {
            ...
            log.info("第三步：获取QQ互联返回的accessTaken="+accessTaken);
        } catch (IOException e) {
            e.printStackTrace();
        }
        return accessTaken;
    ...
    public JSONObject getUserOpenID(String accessToken){
        ...
        try (Response response = client.newCall(request).execute()) {
            ...
            log.info("第四步：获取QQ互联返回的openid和分配给应用的appid："+userInfo);
        } catch (IOException e) {
            e.printStackTrace();
        }
        return userInfo;
    ...
    public JSONObject getUserInfo(String access_token , String oauth_consumer_key , String openid ){
        ...
        try (Response response = client.newCall(request).execute()) {
            ...
            log.info("第五步：获取QQ互联返回的用户有效信息："+userRealInfo);
        } catch (IOException e) {
            e.printStackTrace();
        }
        return userRealInfo;
